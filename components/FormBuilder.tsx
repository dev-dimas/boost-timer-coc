...
const FormBuilder: React.FC<FormBuilderProps> = ({ content }) => {
  ...
  const calculateResult = () => {
    ...
    let totalBoostCapability = boostDuration * 10;
    if (totalMinutes > totalBoostCapability) {
      ...
      spendMinutes = boostDuration - (totalBoostCapability - totalMinutes) / 10;
      setResultMinutes(spendMinutes);
      setBoostDurationLeft((totalBoostCapability - totalMinutes) / 10);
    }
    setShowResult(true);
  };
    ...
    <>
      <form className="mt-4 w-full">
        ...
        <div className="input-group mt-3 flex flex-col">
          <label className="text-center text-[3.5vw] font-bold sm:text-xl">{content.buildTime}</label>
          <div className="flex w-full justify-center rounded-md shadow-sm">
            <input
              type="number"
              className="block w-full rounded-xl rounded-r-none border border-r-0 border-black px-2 py-2 md:w-[50%]"
